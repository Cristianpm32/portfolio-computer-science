...
CuaEncadenada<T>::CuaEncadenada(const CuaEncadenada<T> &q)
{
    ...
    else
    {
        ...
        cout << "L'estructura còpia ha estat creada correctament" << endl;
    }
}
template <typename T>
CuaEncadenada<T>::~CuaEncadenada()
...
void CuaEncadenada<T>::dequeue()
{
    if (isEmpty())
    {
        throw out_of_range("EXCEPTION: La llista de comandes està buida");
    }
...
void CuaEncadenada<T>::print()
{
    if (isEmpty())
    {
        cout << "No hi ha cap comanda registrada";
    }
    else
    {
        cout << "Comandes: " << endl;
